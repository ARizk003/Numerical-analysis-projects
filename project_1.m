...
%dt = 0.25 * dx / (max_vel + 1e-5);  % Add small number to avoid division by zero
    ...
    %density = zeros(Nx, Ny);
    % Initialize fields
  u = ones(Nx)*cos(dt) + ones(Ny)* sin(dt); % Initial velocity to the right (positive x direction)
  v = - ones(Nx) * sin(dt) + ones(Ny) * sin(dt) ;      % No vertical movement
...
%v = strength * v;
    ...
    for t = 1:10000
       ...
       % radius_motion = 50;
         theta = theta + 0.01;
          x_center = 128 +  60 * cos(theta)  ;
          y_center = 128+ (60* sin(2*theta) ) ;


        % === 1. Re-create density blob if needed ===
        blob = ((X-x_center)).^2 + ((Y-y_center)).^2 <  radius.^2;
...
%density(:,end) = 0;   % Right column
    ...
    %       u(1:8:end,1:8:end)', v(1:8:end,1:8:end)', 'w');
    %hold off;
    %axis equal tight off;
    %title(['Time step: ', num2str(t)]);
    %colorbar;
